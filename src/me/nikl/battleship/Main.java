	...
	private void hook() {
		...
		if(config.isConfigurationSection("gameBox.gameButtons")){
			...
			for(String buttonID : gameButtons.getKeys(false)){
				...
				aircraftCarrier = buttonSec.getInt("aircraftCarrier", 1);
				battleship = buttonSec.getInt("battleship", 1);
		...
		if(config.isConfigurationSection("gameBox.topListButtons")) {
			...
			for (String buttonID : topListButtons.getKeys(false)) {
				...
				TopListPage topListPage = new TopListPage(gameBox, guiManager, 54, gameID, buttonID + GUIManager.TOP_LIST_KEY_ADDON, buttonSec.isString("inventoryTitle") ? ChatColor.translateAlternateColorCodes('&', buttonSec.getString("inventoryTitle")) : "Title missing in config", SaveType.WINS, lore);
